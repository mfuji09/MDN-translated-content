...
**`SVGRect`** は、 {{DOMXref("DOMRect")}} の別名であり、四角形を表します。四角形は、最小の `x` の値と最小の `y` の値を特定する `x` と `y` の座標の対と、 `width` と、 `height` から構成されており、これらは、非負であるものと考えられます。
...
- {{domxref("SVGRect.width")}}
  - : これは四角形の幅を表します。負の値はエラーになります。 `0` の値は要素の描画を無効にします。
- {{domxref("SVGRect.height")}}
  - : これは四角形の高さを表します。負の値はエラーになります。 `0` の値は要素の描画を無効にします。
