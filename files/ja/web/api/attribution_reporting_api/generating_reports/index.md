...
この記事では、[帰属レポート API](/ja/docs/Web/API/Attribution_Reporting_API) のレポート（帰属レポートおよびデバッグレポート）が生成される仕組みと、生成されたレポートを制御する方法について説明します。これには、ノイズの処理、レポートの優先順位付け、レポートのフィルター、デバッグレポートの生成などが含まれます。
...
`<reporting-origin>` は、ソースとトリガーを登録したものと同一のオリジンとなります。

レポートデータは JSON 構造に収められています。
...
- `"aggregation_service_payloads"`
  ...
  - : An array of objects representing payload objects containing the histogram contributions used by the aggregation service to assemble the data contained in the report. Currently, only a single payload is supported per report, configured by the browser. In the future multiple, customizable payloads may be supported. Each payload object can contain the following properties:
    ...
    - `"payload"`
      ...
      - : A [CBOR](https://cbor.io/) map encrypted via [HPKE](https://datatracker.ietf.org/doc/rfc9180/) and then [base64](/ja/docs/Glossary/Base64)-encoded, with the following structure:
...
> [!NOTE]
> To use debug reports, the reporting origin needs to set a cookie. If the origin configured to receive reports is a third party, this cookie will be a [third-party cookie](/ja/docs/Web/Privacy/Guides/Third-party_cookies), which means that debug reports will not be available in browsers where third-party cookies are disabled/not available.
