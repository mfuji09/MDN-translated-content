...
その結果、次の `pointermove` イベントを受信するまで、指定したスタイルで、アプリに代わって、既定のブラウザーのレンダリングの前に、委任されたインクトレイルが描画されます。
