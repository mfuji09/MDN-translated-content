...
To use this method, the user must grant permission to access `local-fonts` (permission status can be queried via the {{domxref("Permissions API", "", "", "nocode")}}). In addition, this feature may be blocked by a [権限ポリシー](/ja/docs/Web/HTTP/Permissions_Policy) set on your server.
...
- `SecurityError` {{domxref("DOMException")}}
  - : Use of this feature was blocked by a [権限ポリシー](/ja/docs/Web/HTTP/Permissions_Policy), or it was not called via a user interaction such as a button press, or current {{glossary("origin")}} is an opaque origin.
