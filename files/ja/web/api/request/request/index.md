...
- `input`
  ...
  - : 取得したいリソースを定義します。次のいずれかの値を取ります。
    ...
    - 取得したいリソースのURLを収めた文字列。URLは、ベースURLに対する相対URLである可能性があります。ベースURLとは、ウィンドウコンテキストでは文書内の {{domxref("Node.baseURI", "baseURI")}}、ワーカーコンテキストでは {{domxref("WorkerGlobalScope.location")}} です。
...
- `options` {{optional_inline}}
  ...
  - : リクエストに適用するカスタム設定を含む {{domxref("RequestInit")}} オブジェクトです。

    既存の `Request` から新しい `Request` を構築する場合、新しいリクエストの _options_ 引数にオプションを設定すると、元の `Request` に設定された対応するオプションを置き換えます。次のようになります。
...
この場合、どのようなキャッシュされたレスポンスでも問題ないことを示す {{httpheader("Cache-Control")}} 値を設定するには、以下のようにします。

```js
const myImage = document.querySelector("img");
const reqHeaders = new Headers();

// キャッシュされたレスポンスは、1週間以上経過していなければ問題ない
...
// 初期化内容を "options" オブジェクトとして、ヘッダーとともに渡す
