...
CSSで色を表すには、「色」というアナログの概念をコンピューターが使用することができるデジタルの形に変換する方法を探さなければなりません。これは通常、色をさまざまな原色の混合量や明度、色相などの部分に分解することで行われます。定義する色モデルは、色がどこでレンダリングされても同じように現れることを保証します。

色モデルとは、色を数値で表す数学的モデルのことです。色モデルは、色空間内で利用できる色を作成する方法を記述します。 {{glossary("RGB")}} はウェブの最初の色モデルでした。 RGB 色モデルのsRGB色空間（標準の赤、緑、青の色空間）は、 1996 年にコンピューターのモニターとウェブ用に作成されました。{{glossary("color space","色空間")}}とは、指定された色を一貫して記述できるように、色をグループ化するシステムのことです。 2 つの異なる色空間の間で色を変換した場合、どちらの色空間でも同じように見えるはずです。

元々、モニターは表示できる色の数に制限があり、 CSS の色もその制限によって制限されていました。現在の端末は RGB に制限されなくなりました。これで人間の知覚に基づいた色モデルも利用できるようになり、より広範な{{glossary("gamut","色域")}}を提供することができるようになりました。これで、 CSS で色を記述する方法がいくつかできるようになり、選択肢はどんどん拡張されています。

このガイドでは、さまざまな {{cssxref("&lt;color&gt;")}} 値型を紹介します。より詳細な説明については、下記に指定された参照リンクを参照してください。

## キーワード

ウェブでは、色を説明するために数値表現の代わりにキーワードを使用できる標準色名のセットを定義しています。これは、より単純ではありますが、より制限の多い手法です。使用したい色を正確に表すキーワードがあるとは限りません。

色キーワード には、 標準原色や二次色 （ `red`, `blue`, `orange` など）、 グレーの濃淡（`black` から `white` までで、 `darkgray` や `lightgray` などの色を含む）、そのほか `lightseagreen`、`cornflowerblue`、`rebeccapurple` などのさまざまな混色が含まれます。名前付き色は [RGB](/ja/docs/Glossary/RGB) モデルを使用して、 sRGB (`srgb`) 色空間に関連付けられています。

名前付き色は 160 以上があります。特に興味深い名前付き色としては、 [`transparent`](/ja/docs/Web/CSS/named-color#transparent) は透明色の値を設定し、 [`currentcolor`](/ja/docs/Web/CSS/color_value#currentcolor_keyword) は CSS の {{cssxref("color")}} プロパティの現在の値を設定します。また、名前付き {{cssxref("system-color")}} の色、たとえば `accentcolortext` や `buttonface` などもあり、ユーザーやブラウザー、オペレーティングシステムによる既定色の選択を反映します。

色キーワードはすべて大文字と小文字を区別しません。色キーワードの詳細については {{cssxref("named-color")}} データ型を参照してください。

## RGB 値

CSS において {{glossary("RGB")}} 色を赤、緑、青の部分で定義する方法には、主に 16 進数と `rgb()` 値の 2 つがあります。名前付き色と同様に、これらの方法は [RGB](/ja/docs/Glossary/RGB) モデルを使用し、 sRGB (`srgb`) 色空間に関連付けられます。しかし、より広い範囲の色を指定することができます。

### 16 進文字列記法

16進 (hex) 文字列記法は、RGB色の各成分（赤、緑、青）を表すのに 16 進値を使用します。また、4番目の成分としてアルファチャンネル（または不透明度）を記載することもできます。

16 進文字列記法の色は常に文字 `"#"` で始まります。その後に色コードの 16 進数が続きます。文字列は大文字と小文字を区別しません。
...
The following JavaScript updates the border's color to match the color picker's initial value, then adds two event handlers to the [`<input type="color">`](/ja/docs/Web/HTML/Element/input/color) element to respond to changes made to its value.
...
If you want explicit control over color spaces when defining colors, you can use the [`color()`](/ja/docs/Web/CSS/color_value/color) function.

This is useful to describe a color for high-definition devices with wider color [gamuts](/ja/docs/Glossary/Gamut).
For example, if you wanted to show the `display-p3 0 0 1` color, which is outside of the sRGB gamut, you could use a `@media` [`color-gamut`](/ja/docs/Web/CSS/@media/color-gamut) at-rule to detect if the client's hardware supports colors in this range before trying to use it:
...
- [Using color wisely](/ja/docs/Web/CSS/CSS_colors/Using_color_wisely)
- [Using relative colors](/ja/docs/Web/CSS/CSS_colors/Relative_colors)
- [Understanding color and luminance](/ja/docs/Web/Accessibility/Understanding_Colors_and_Luminance)
- [WCAG 1.4.1: Color contrast](/ja/docs/Web/Accessibility/Understanding_WCAG/Perceivable/Color_contrast)
- [CSS color module](/ja/docs/Web/CSS/CSS_colors)
