...
これにより、12 の太さのリストが作成されます。`column-rule-width` の値に含まれる太さのリストにある太さの数が、列・段間の数を超えた場合、余分な太さの値は無視されます。コンテナーが 3 列の場合、1 つ目の列・段間に適用される罫線は `1rem` の太さ、2 つ目の列・段間に適用される罫線は {{cssxref("min()")}} 関数で判断されます。
